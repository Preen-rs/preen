                    ...
                    Event::Mouse(mouse) => {
                        ...
                        match mouse.kind {
                            ...
                            MouseEventKind::Down(MouseButton::Left) => {
                                ...
                                if let Some(view) =
                                    ui::sidebar_view_at(area, mouse.column, mouse.row)
                                {
...
fn read_event_batch() -> Result<Vec<Event>, String> {
    const MAX_EVENT_BATCH: usize = 64;

    let mut events = Vec::with_capacity(8);
    events.push(event::read().map_err(|error| error.to_string())?);
